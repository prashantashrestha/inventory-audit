...
(async () => {
  try {
    const poolName = await fetchPools();
    const vmDetails = await fetchVms();
    //Map pool name to vm details
    const newVmDetails = Object.fromEntries(
     Object.entries(vmDetails).map(([uuid,vmList]) => [
      poolName[uuid],vmList
     ])
    );
    fs.writeFileSync("xo_vms.json",JSON.stringify(newVmDetails, null, 2));
  } catch (err) {
...
// Fetch all Pool Names
async function fetchPools() {
    try {
        const url = `${XO_HOST}/rest/v0/pools?fields=name_label,uuid`;
        const hosts = await fetchWithAuth(url);
        const hostsInfo = {};
        for (const host of hosts) {
            hostsInfo[host.uuid] = host.name_label
        }
	return hostsInfo;
...
async function fetchVms() {
  try {
    const url = `${XO_HOST}/rest/v0/vms?fields=$pool,name_label,mainIpAddress,power_state`;
    const vms = await fetchWithAuth(url);
    const vmLookup = vms.reduce((acc, vm) => {
      if (!acc[vm.$pool]) {
        acc[vm.$pool] = [];
      }
